...
public:
    ...
    protected:
        iterator() : m_p_st(nullptr) {}
        explicit iterator(Status& st, bool is_end = false) : m_p_st(&st)
        {
            if (is_end) {
    ...
    public:
        iterator operator++(int)
        {
            iterator ret(*this);
            if (m_p_st &&
                std::distance(this->m_beg_it, m_p_st->m_begins.end()) > 0) {
                m_beg_it++;
                m_end_it++;
            }
            return ret;
        ...
        iterator operator--(int)
        {
            iterator ret(*this);
            if (m_p_st &&
                std::distance(m_p_st->m_begins.begin(), this->m_beg_it) > 0) {
                m_beg_it--;
                m_end_it--;
            }
            return ret;
    ...
    int tabstop() const { return this->m_tabstop; }
    int tabstop(int ts)
    {
    ...
    bool is_init() const { return !this->m_begins.empty(); }
    size_t size() const { return this->m_begins.size(); }
    ...
    {
        return this->is_init() && this->m_begins.front() <= it_beg &&
               this->m_ends.back() >= it_end;
    }

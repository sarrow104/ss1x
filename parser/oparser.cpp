...
namespace ss1x {
    namespace parser{
        ...
        // 当然，还可以用delayeval
        const rule::ActionT rule::null_action
            = [](StrIterator, StrIterator, rule::matched_value_t)->void { };
        ...
        const char * rule::get_type_name(type_t t)
        {
            ...
            if (name_table.empty()) {
#define DO_ADD(a)  case a: name_table[a] = #a; break;
...
#undef DO_ADD
        ...
        // 因为这本来就是默认行为；
        bool rule::match (StrIterator& it_beg, StrIterator it_end, delayeval& d, Status * p_st) const
        {
            if (p_st) {
                p_st->init(it_beg, it_end);
            }
        ...
        {
            // std::cout << "@" << this << " m_anchor_used = " << this->m_anchor_used << std::endl;
            Status st;
            // NOTE
            // this->m_anchor_used
            // 这个量，其实是不准的！
        ...
        bool rule::check_full (StrIterator it_beg, StrIterator it_end, Status * p_st) const
        {
            // std::cout << "@" << this << " m_anchor_used = " << this->m_anchor_used << std::endl;
            Status st;
            if (this->m_anchor_used) {
        ...
        bool rule::do_match (StrIterator& it_beg, StrIterator it_end,
                             bool no_action, delayeval* p_delay, Status * p_st) const
        {
            ...
            util::delayeval_helper dh(this, it_beg,
                                      this->m_action, user_data,
                                      no_action, p_delay,
                                      p_st,
                                      ret);
            ...
            case RULE_TYPE_ANYCHAR: // {{{1
                {
                    ...
                    if (ret && m_action) {
                        if (m_matched_action.which()) {
                            util::assign_result(this, user_data, m_action, m_matched_action, it_beg_sv, it_beg);
                        }
                        else {
            ...
            case RULE_TYPE_RANGE: // {{{1
                {
                    uint32_t ch = 0;
                    ret = Parser::parseRangeUtf8(it_beg, it_end,
                                                 this->m_data.ch_range[0], this->m_data.ch_range[1],
                                                 ch);
                    if (ret && m_action) {
                        if (m_matched_action.which()) {
                            util::assign_result(this, user_data, m_action, m_matched_action, it_beg_sv, it_beg);
            ...
            case RULE_TYPE_CHARSET: // {{{1
                {
                    ...
                    if (st.second) {
                        const auto set_it = std::lower_bound(this->m_setdata.cbegin(),
                                                             this->m_setdata.cend(),
                                                             st.first);
            ...
            case RULE_ANCHOR_COL_IF_TIMES:
                ...
                }
                {
                    ...
                    while (this->m_subs[0].do_match(it_beg, it_end, no_action, dh.get_delay_ptr(), p_st)) {
            ...
            case RULE_OPERATOR_MOSTONETIME: // {{{1
                {
                    Parser::Rewinder r(it_beg);
                    assert(this->m_subs.size() == 1);
                    r.commit(this->m_subs[0].do_match(it_beg, it_end, no_action, dh.get_delay_ptr(), p_st));
            ...
            case RULE_OPERATOR_DIFFERENCE: // A not B
                {
                    ...
                    if (this->m_subs[0].do_match(it_beg_togo, it_end, no_action, dh.get_delay_ptr(), p_st)) {
                             ...
                             ++i)
                        {
                            ...
                            if (this->m_subs[i].do_match(it_beg_fail, it_end, true, 0, p_st)) {
                                has_failed = true;
                                break;
                            }
                        }
            ...
            case RULE_OPERATOR_PLUSTIMES: // {{{1
                {
                    Parser::Rewinder r(it_beg);
                    assert(this->m_subs.size() == 1);
                    if (r.commit(this->m_subs[0].do_match(it_beg, it_end, no_action, dh.get_delay_ptr(), p_st))) {
                        while (true) {
                            r.begin();
                            r.commit(this->m_subs[0].do_match(it_beg, it_end, no_action, dh.get_delay_ptr(), p_st));
                            if (!r.is_commited()) {
            ...
            case RULE_OPERATOR_RANGETIMES: // {{{1
                {
                    ...
                    if (low) {
                        StrIterator it_beg_sv = it_beg;
                        while (low && this->m_subs[0].do_match(it_beg_sv, it_end,
                                                               no_action, dh.get_delay_ptr(), p_st))
                        {
                            low--;
                            if (high > 0) {
                                high--;
                    ...
                    if (low == 0) {
                        while (high != 0) {
                            r.begin();
                            r.commit(this->m_subs[0].do_match(it_beg, it_end,
                                                              no_action, dh.get_delay_ptr(), p_st));
                            if (!r.is_commited()) {
                                break;
                            }
            ...
            case RULE_OPERATOR_BRANCH: // {{{1
                {
                    ...
                    for (size_t i = 0; i != this->m_subs.size(); ++i) {
                        r.commit(this->m_subs[i].do_match(it_beg, it_end,
                                                          no_action, dh.get_delay_ptr(), p_st));
                        if (r.is_commited()) {
                            match_succeed = true;
                            break;
            ...
            case RULE_OPERATOR_CATENATE: // {{{1
                {
                    ...
                    for (size_t i = 0; i != this->m_subs.size(); ++i) {
                        if (!this->m_subs[i].do_match(it_beg, it_end,
                                                      no_action, dh.get_delay_ptr(), p_st)) {
                            match_failed = true;
                            break;
                        }
            ...
            case RULE_OPERATOR_MUST_FOLLOWED: // {{{1
                {
                    ...
                    if (this->m_subs[0].do_match(it_beg, it_end,
                                                 no_action, dh.get_delay_ptr(), p_st))
                    {
                        for (size_t i = 1; i != this->m_subs.size(); ++i) {
                            ...
                            {
                                if (!no_action) {
                                    StrIterator sample_end = std::find(it_beg, it_end, '\n');
                                    ...
                                    // 我的建议：
                                    // 1. 做一个输出规则用对象包裹器，即，在输出的时候，不跟踪引用；
                                    // 2. 一个通用的裁剪工具——只输出部分，比如一行。
                                    //    std::ostream 的 setw()有用没有？
                                    //    好像对这里没用。
                                    //    因为转嫁的是rule::print, -> visitor……
                                    // 3. 做一个深度限制。
                                    SSS_POSTION_THROW_3(ErrorPosition,
                                                        "@" << this->m_subs[i].get_ptr()
                                                        << " from: " << this->get_type_name(this->m_type)
                                                        << " @" << this
                                                        << "\nleft sample=`" << sss::util::make_slice(it_beg, sample_end)
                                                        << "`",
                                                        this->m_subs[i].get_ptr(),
                                                        it_beg, coord);
                                }
            ...
            case RULE_OPERATOR_SEQUENTIAL: // {{{1
                ...
                // 所以，这个式子，等于： (a >> -b) | b; 另外，-b 是 匹配 b 0-1次的意思……
                ...
                // -> (a >> -b | b) >> -c | c
                {
                    Parser::Rewinder r(it_beg);
                    assert(this->m_subs.size() >= 2);
                    bool is_prev_ok = r.commit(this->m_subs[0].do_match(it_beg, it_end,
                                                                        no_action, dh.get_delay_ptr(), p_st));
                    for (size_t i = 1; i != this->m_subs.size(); ++i) {
                        Parser::Rewinder r2(it_beg);
                        Parser::Rewinder r3(it_beg);
                        is_prev_ok =
                            r2.commit(is_prev_ok &&
                                      ((r3.begin() &&
                                        r3.commit(this->m_subs[i].do_match(it_beg, it_end,
                                                                           no_action, dh.get_delay_ptr(), p_st))) || true))
                            ||
                            r2.commit(this->m_subs[i].do_match(it_beg, it_end,
                                                               no_action, dh.get_delay_ptr(), p_st));
                    }
                    ret = is_prev_ok;
                    if (ret) {
            ...
            case RULE_OPERATOR_SEP_SUGAR: // a % b {{{1
                {
                    ...
                    Parser::Rewinder r2(it_beg);
                    assert(this->m_subs.size() == 2);
                    if (r.commit(this->m_subs[0].do_match(it_beg, it_end,
                                                          no_action, dh.get_delay_ptr(), p_st)))
                    {
                        while (true) {
                            r2.begin();
                            r2.commit(this->m_subs[1].do_match(it_beg, it_end, true, 0, p_st) &&
                                      this->m_subs[0].do_match(it_beg, it_end,
                                                               no_action, dh.get_delay_ptr(), p_st));
                            if (!r2.is_commited()) {
                                break;
                            }
            ...
            case RULE_OPERATOR_REFER: // {{{1
                ...
                ret = this->m_data.ref->do_match(it_beg, it_end, no_action, dh.get_delay_ptr(), p_st);
                if (ret) {
                    util::assign_result(this, user_data, m_action, m_matched_action, it_beg_sv, it_beg);
                }
                break;
            ...
            std::string msg = ret ? "`" + std::string(it_beg_sv, it_beg) + "`" : std::string("failed");
            SSS_LOG_DEBUG("@%p, (%d) %s\n", this, ret, msg.c_str());
        ...
        rule make_by_type(rule::type_t t)
        {
            return rule(t);
        }
        ...
        const rule cidentifier_p
            = ((parser::alpha_p | parser::char_p('_')) >> *(parser::alnum_p | parser::char_p('_')))

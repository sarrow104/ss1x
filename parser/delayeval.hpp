 ...
 * ，修改为动作序列的push；
 * 因此真正的match动作，需要额外两个控制参数，一个是delayeval指针，一个是bool no_action
 *
 * public:
 *   bool rule::match(Iterator& it_beg, Iterator it_end);
 *   bool rule::match(Iterator& it_beg, Iterator it_end, delayeval& delay);
 *   bool rule::check(Iterator it_beg, Iterator it_end); // 检查；不触发动作，也不延迟
 *   bool rule::check_full(Iterator it_beg, Iterator it_end); // 检查是否完全匹配——无剩余
 *
 * protected:
 *   rule::do_match(Iterator& it_beg, Iterator it_end, bool no_action, delayeval* p_delay);
 *
 */
...
namespace parser {
    ...
    public:
        ...
        bool     is_init() const
        {
            return bool(this->m_rul_ptr);
        }

        void    set_end_pos(StrIterator it_end)
        {
        ...
        delayeval& operator[] (int i) {
            return this->m_subs[i];
        }
        ...
        template<typename T>
        T getUserData () const {
            // std::cout << __func__ << " " << this->m_usr_data.which() << std::endl;
            // std::cout << sss::util::make_slice(m_it_beg, m_it_end) << std::endl;
            return boost::get<T>(this->m_usr_data);
        }

        rule::matched_value_t get_userdata() const {
            return this->m_usr_data;
        }

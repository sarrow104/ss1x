...
namespace parser {
    ...
    public:
        ...
        struct empty { }; // empty value for boost::variant
...
#undef MATCHED_VALUE_T_LIST
        ...
        typedef std::function<void(StrIterator, StrIterator, matched_value_t)> ActionT;

        typedef std::function<bool(StrIterator&, StrIterator, util::delayeval_helper*)> DoMatchActionT;
    ...
    public:
        rule()
        {
            this->init_before(RULE_TYPE_ANYCHAR);
        }
...
//                 | this->m_type == RULE_ANCHOR_COL_IF_TIMES;
    ...
    public:
        ...
        bool match (StrIterator& it_beg, StrIterator it_end, delayeval& d, Status * p_st = 0) const;
        ...
        bool check_full (StrIterator it_beg, StrIterator it_end, Status * p_st = 0) const;
    ...
    inline rule char_set_p(const char * s)
    {
        return utf8_set_p(s);
    }
    ...
    inline rule utf8_range_p(const char * utf8_low, const char * utf8_high)
    {
        ...
        std::tie(ret.m_data.ch_range[0], len1) =
            sss::util::utf8::peek(utf8_low);
        std::tie(ret.m_data.ch_range[1], len2) =
            sss::util::utf8::peek(utf8_high);
    ...
    inline int match(const parser::rule& r, const std::string& s, ss1x::parser::delayeval &d) {
    ...
    inline rule dict_p(dictionary& d)
    {
        return rule(d);
    }

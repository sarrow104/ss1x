...
namespace parser {
    ...
    {
    public:
        bool operator () (const DictionaryTree::slot_t& lhs, const DictionaryTree::slot_t& rhs)
        {
    ...
    public:
        ...
        {
            return
                !sss::stricmp_t()(lhs.m_word, rhs.m_word) &&
    ...
    void DictionaryTree::calcPrefix()
    {
        ...
        for (size_t i = 0; i + 1 != this->m_list.size(); ++i) {
            ...
            for (size_t next = i + 1; next != this->m_list.size(); ++next) {
                if (!sss::is_begin_with(this->m_list[next].m_word, this->m_list[i].m_word, this->m_is_ic)) {
                    break;
                }
    ...
    bool        KeywordsTree::match(StrIterator& it_beg, StrIterator it_end, int & value)
    {
        ...
        if (this->m_rule.match(it_beg_sv, it_end)) {
            ...
            if (this->m_is_ic) {
                ...
                if (it != this->m_list.end() && slot_equal_ic()(*it, slot_t(slot))) {
                    value = it->m_value;
                    it_beg = it_beg_sv;
                    return true;
                }
                ...
                if (it != this->m_list.end() && slot_equal_cs()(*it, slot_t(slot))) {
                    value = it->m_value;
                    it_beg = it_beg_sv;
                    return true;
                }
...
#else
            int t_value = 0;
            if (this->DictionaryTree::match(t_beg, t_end, t_value) && t_beg == t_end) {
                value = t_value;
                it_beg = it_beg_sv;
                return true;
            }

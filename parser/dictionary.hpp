...
namespace parser {
    ...
    protected:
        ...
        template<typename CompT> bool match(StrIterator it_beg, StrIterator it_end,
                                            DataIterator& max_match_iter,
                                            CompT func)
        {
            ...
            DataIterator first = std::lower_bound(this->m_list.begin(),
                                                  this->m_list.end(),
                                                  slot_t(first_try, 0),
                                                  func);
            ...
            first_try += '\xff';
            DataIterator last = std::upper_bound(this->m_list.begin(),
                                                 this->m_list.end(),
                                                 slot_t(first_try, 0),
                                                 func);
            ...
            {
                {
                    // std::cout << first->m_word << " : from " << current_match_cnt;

                    current_match_cnt += sss::max_match(it_beg + current_match_cnt, it_end,
                ...
                if (max_match_cnt < current_match_cnt) {
                    // std::cout << " extent max_match_cnt from " << max_match_cnt << " to " << current_match_cnt << std::endl;
        ...
        template<typename CompT> void add(const std::string& slot, int value, CompT func)
        {
            if (!std::binary_search(this->m_list.begin(),
                                    this->m_list.end(),
                                    slot_t(slot, value),
                                    func))
            {
        ...
        template<typename CompT> void remove(const std::string& slot, CompT func)
        {
            typedef std::vector<slot_t>::iterator DataIterator;
            DataIterator first = std::lower_bound(this->m_list.begin(),
                                                  this->m_list.end(),
    ...
    {
    public:
        explicit KeywordsTree(const ss1x::parser::rule = ss1x::parser::cidentifier_p, bool is_ic = false);
        ~KeywordsTree();
    ...
    public:
        explicit KeywordsList(const ss1x::parser::rule = ss1x::parser::cidentifier_p, bool is_ic = false);

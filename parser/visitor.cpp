...
void printable_char(std::ostream& o, char c)
{
    if (::isprint(c)) {
        ...
        o << "\\x" << std::hex << std::setfill('0') << std::setw(2)
          << uint32_t(c);
    }
}
...
#undef DO_ADD
    ...
    switch (r->m_type) {
        ...
        case rule::RULE_TYPE_CHARSET:
            ...
            {
                std::string s;
                sss::util::utf8::dumpout2utf8(r->m_setdata.cbegin(),
                                              r->m_setdata.cend(),
                                              std::back_inserter(s));
        ...
        case rule::RULE_OPERATOR_REFER: {
            ...
            if (!this->m_no_refer &&
                this->m_has_entered.find(r->m_data.ref) ==
                    this->m_has_entered.end()) {
    ...
    if (!this->m_no_refer && this->m_indent == 0 &&
        !this->m_tobe_entered.empty()) {
        this->o() << std::endl;
        this->m_first = true;

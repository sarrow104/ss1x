...
struct has_type<T, First, Types...> {
    static constexpr bool value =
        std::is_same<T, First>::value || has_type<T, Types...>::value;
};
...
struct variant_helper<T, Types...> {
    ...
    VARIANT_INLINE static void move(const std::size_t old_id, void* old_value,
                                    void* new_value)
    {
    ...
    VARIANT_INLINE static void copy(const std::size_t old_id,
...
struct binary_dispatcher_rhs<F, V, R, T0, T1, Types...> {
    using result_type = R;
    VARIANT_INLINE static result_type apply_const(V const& lhs, V const& rhs,
                                                  F f)
    {
    ...
    VARIANT_INLINE static result_type apply(V& lhs, V& rhs, F f)
    {
        if (rhs.get_type_index() == sizeof...(Types))  // call binary functor
        {
            ...
            return binary_dispatcher_rhs<F, V, R, T0, Types...>::apply(lhs, rhs,
                                                                       f);
        }
    }
};
...
struct binary_dispatcher_lhs<F, V, R, T0, T1, Types...> {
    ...
    VARIANT_INLINE static result_type apply(V& lhs, V& rhs, F f)
    {
        if (lhs.get_type_index() == sizeof...(Types))  // call binary functor
        {
            ...
            return binary_dispatcher_lhs<F, V, R, T0, Types...>::apply(lhs, rhs,
                                                                       f);
        }
    }
};
...
struct binary_dispatcher<F, V, R, T, Types...> {
            ...
            if (v0.get_type_index() == v1.get_type_index()) {
                return f(unwrapper<T>::apply_const(v0.template get<T>()),
                         unwrapper<T>::apply_const(
                             v1.template get<T>()));  // call binary functor
            }
            else {
    ...
    VARIANT_INLINE static result_type apply(V& v0, V& v1, F f)
    {
        if (v0.get_type_index() == sizeof...(Types)) {
            ...
            return binary_dispatcher_lhs<F, V, R, T, Types...>::apply(v0, v1,
                                                                      f);
        }
...
struct binary_dispatcher<F, V, R, T> {
    ...
    {
        return f(
            unwrapper<T>::apply(v0.template get<T>()),
...
public:
                     ...
                     typename std::remove_reference<T>::type, Types...>::index)
    {
        constexpr std::size_t index =
            sizeof...(Types)-detail::value_traits<
                typename std::remove_reference<T>::type, Types...>::index -
            1;
        using target_type =
...
public:
    ...
    {
        static_assert(detail::has_type<T, Types...>::value,
                      "invalid type in T in `is<T>()` for this variant");
    ...
    // unary
    template <typename F, typename V>
    auto VARIANT_INLINE static visit(V const& v, F f) -> decltype(
        detail::dispatcher<
            F, V, typename detail::result_of_unary_visit<F, first_type>::type,
                ...
                Types...>::apply_const(v0, v1, f))
    {
        ...
        return detail::binary_dispatcher<F, V, R, Types...>::apply_const(v0, v1,
                                                                         f);
    }
    // non-const
    template <typename F, typename V>
    auto VARIANT_INLINE static binary_visit(V& v0, V& v1, F f) -> decltype(
        detail::binary_dispatcher<
            F, V, typename detail::result_of_binary_visit<F, first_type>::type,
            Types...>::apply(v0, v1, f))
    {
        using R = typename detail::result_of_binary_visit<F, first_type>::type;
    ...
    VARIANT_INLINE bool operator==(variant const& rhs) const
    {
        ...
        if (this->get_type_index() != rhs.get_type_index()) return false;

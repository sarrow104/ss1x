...
#define USER_AGENT_DEFAULT "Mozilla/5.0 (Windows; U; Windows NT 5.1; zh-CN; rv:1.9.1.6) Gecko/20091201 Firefox/3.5.6\r\n"
...
namespace  {
     ...
     * 2. post命令
     * 3. 文件路径
     * Content-Type: multipart/form-data; boundary=---------------------------2473929242097947597857883638
     * 另外，还可以post xml、json；以及url命令参数；
     * postXMLInner
     * postJSONInner
     * postParamsInner
     */
    void postFileInner(ss1x::http::Headers* headers,
...
#undef VALUE_MSG
    ...
    void getFileInner(std::ostream&      outFile,
...
void getFile(std::ostream&      outFile,
             const std::string& serverName,
             const std::string& getCommand,
             int                port)
{
...
void proxyGetFile(std::ostream&     outFile,
                  ...
                  int                port)
{
    ...
    ss1x::asio::getFile(outFile, header, proxy_domain, proxy_command, proxy_port);
}
